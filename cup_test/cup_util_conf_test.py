...
import os
import sys
_TOPDIR = os.path.abspath(os.path.dirname(os.path.abspath(__file__)) + '/../')
sys.path.insert(0, _TOPDIR)
_TESTDIR = _TOPDIR + '/cup_test'
import unittest
...
class TestUtilConf(unittest.TestCase):
    ...
    def test_arrary_special_chars(self):
        """test arrary"""
        # test conf arrary
        list_conf = conf.Configure2Dict(_TESTDIR + '/conf/test.list.conf', False).get_dict()
        ...
        confdict = conf.Configure2Dict(_TESTDIR + '/conf/conf_mock.conf', False).get_dict()
        ...
        confobj.write_conf(_TESTDIR + '/conf/test.conf')

        test_dict = conf.Configure2Dict(_TESTDIR + '/conf/test.conf').get_dict()
        print('test_comments:', confdict['test_comments'])


    def test_include_files(self):
        """test include"""
        include_conf = conf.Configure2Dict(_TESTDIR + '/conf/test.include.conf')
        include_dict = include_conf.get_dict(ignore_error=True)
        ut.assert_in(
            './test.included.conf', include_dict['$include'].keys()
        )
        in_confdict = include_dict['$include']['./test.included.conf']
    ...
    def test_include_wrong_file(self):
        """test wrong include"""
        include_conf = conf.Configure2Dict(_TESTDIR + '/conf/test.include.wrong.conf')
        include_dict = include_conf.get_dict(ignore_error=True)
        ut.assert_in(
            './test.included.conf', include_dict['$include'].keys()
        )
        ...
        conf.Dict2Configure(include_dict).write_conf(_TESTDIR + '/conf/test.include.write')
        try:
